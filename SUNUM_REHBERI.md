...
Süpervize duruş-tahmini için dört model ailesini ve iki saçma tabanı aynı held-out gelecekte yarıştırdık.
Karşılaştırma birleşik veride yapıldı (taban oran yüzde 25), sonuçlar `analysis/reports/03_PREDICTION_BENCHMARK.md`
içinde kayıtlıdır.

| Model ailesi | ROC | Lift | Yorum |
|---|---:|---:|---|
| HistGBDT (gradyan artırma) | 0,76 | 2,09 | Seçilen aile |
...
Ayrıca farklı eğitim rejimlerini de karşılaştırdık (`analysis/artifacts/regime_metrics.json`):

| Eğitim verisi | Test | Taban oran | ROC | Lift | Recall |
|---|---|---:|---:|---:|---:|
| Fanuc (ayarlanmış) | Fanuc | 0,18 | 0,73 | 2,38 | 0,69 |
| Fanuc (temel) | Fanuc | 0,18 | 0,68 | 2,0 | 0,59 |
| Birleşik, z-norm | Fanuc | 0,18 | 0,72 | 2,33 | 0,48 |
| Mitsubishi | Mitsubishi | 0,53 | 0,61 | 1,2 | 0,97 |

Bu tablolardan çıkan sonuçlar:

- HistGBDT diğer algoritmaları (Random Forest, Lojistik Regresyon, MLP) geçti, bu yüzden seçildi.
...
Sunum 14 slayttır. Renk kimliği tek bir koyu yeşildir, Mitsubishi makineleri turuncu ile ayrılır. Her
slaytta sol kenarda ince yeşil bir şerit, üstte küçük yeşil bir bölüm etiketi ve büyük bir başlık vardır.
Altta sabit bir alt bilgi ve sayfa numarası bulunur.
...
- Grafik: yatay çubuklarla yedi makinenin OEE değeri, büyükten küçüğe sıralı. Yeşil çubuk Fanuc, turuncu
...
- Sağdaki kutu: üç maddeyle grafiği yorumlar. Ana mesaj, tüm makinelerde OEE'nin pratikte Kullanılabilirlik
...
### Slayt 6: Tahmin Modeli ve Rejim Karşılaştırması (Tablo var)
- Üstte dört özet kart: ROC 0,73, Lift 2,38, Recall yüzde 69, Mitsubishi lift 1,2.
- Tablo: farklı eğitim rejimlerinin (Fanuc ayarlı, Fanuc temel, birleşik z-norm, Mitsubishi) ROC, lift,
  recall karşılaştırması. İlk satır (dağıtılan Fanuc modeli) yeşil vurgulu.
...
- Bu slayt, "hangi makineleri tahmin edebiliyorsunuz" sorusunun yanıtıdır.

### Slayt 7: Algoritma Seçimi (Tablo var)
- Tablo: dört model ailesinin (HistGBDT, Random Forest, Lojistik Regresyon, MLP) ve iki saçma tabanın
  aynı held-out gelecekteki ROC ve lift karşılaştırması. HistGBDT satırı yeşil vurgulu.
- Altta yeşil kutu: HistGBDT'nin neden seçildiği. En yüksek PR-AUC ve ROC, eksik değeri yerel işlemesi,
  ölçekleme gerektirmemesi, doğrusal olmayan eşikleri yakalaması. CNN ve transformer yalnızca denetimsiz
  anomali otokodlayıcısında denendi, tahmin için avantaj sağlamadı.
- Bu slayt, "hangi algoritmaları denediniz ve neden HistGB" sorusunun yanıtıdır.

### Slayt 8: Risk Zaman Çizgisi (Grafik var)
...
### Slayt 9: Tahminden OEE Kazanımına (Köprü)
...
### Slayt 10: Makineler Arası Örüntü (Grafik var)
- Grafik: üç yatay çubuk. Gözlemlenen 708 saat (yeşil), vardiya beklentisi 564 saat (turuncu), rastgele
...
### Slayt 11: What-If Senaryoları (Tablo var)
...
- Planlı duruşlar Kullanılabilirliği düşürmez, doğrudan paydayı küçültür. Bu yüzden bir duruşu plansızdan
  planlıya taşımak A'yı yükseltir.

### 3.3 Model ve sızıntı koruması
- Dağıtılan model HistGBDT'dir, Fanuc hücresinde 41 öznitelik kullanır. Öznitelikler yalnızca geçmiş
  bilgisinden türetilir.
- Bölme kronolojiktir ve makine bazlıdır (per-machine chronological), train_frac 0,6. Karıştırma yoktur,
  en eski yüzde 60 eğitime, en geç yüzde 40 teste gider.
- Model karşılaştırması birleşik veride 58 öznitelikle yapıldı (eğitim yaklaşık 234 bin, test 156 bin).
- Normalizasyon, imputer ve ölçekleyici istatistikleri yalnızca eğitim verisinden hesaplanır, böylece
  test bilgisi sızmaz.
- Aktif bir önemli duruşun içindeki kovalar çıkarıldı, böylece zaten olan bir duruşu tahmin etmedik. Bu
  kovaları çıkarmak skoru yükseltti, bu da liftin gerçek duruş öncesi sinyalden geldiğini kanıtlar.
- Izgara araması (rastgele arama) en iyi parametreleri iç çapraz doğrulamayla seçti. En iyi konfigürasyon:
  öğrenme oranı 0,02, en fazla 300 ağaç, en fazla 15 yaprak, yaprak başına en az 200 örnek, L2
  düzenlileştirme 1,0.
...
- "Modeli neden tüm makinelere uygulamadınız?" Çünkü yalnızca Fanuc hücresinde lift 1'in belirgin
  ...
  tarafında tuttuk.
- "Neden HistGBDT, MLP veya başka bir model değil?" Çünkü aynı held-out gelecekte HistGBDT, Random Forest,
  Lojistik Regresyon ve MLP'yi PR-AUC ve ROC'ta geçti. MLP en zayıf öğrenen oldu, çünkü bu tür tablo ve
  eksik değer ağırlıklı veride gradyan artırma sinir ağını geçer.
