...
INK = HexColor("#16201C"); GREEN = HexColor("#0F7A52"); GREEN_SOFT = HexColor("#ECF4EF")
MUTED = HexColor("#6B776F"); HAIR = HexColor("#D9DFDB")
RED = HexColor("#C43D3D"); AMBER = HexColor("#B7791F"); GRAY = HexColor("#9AA8A1")
...
def numbox(items, top, h=1.45, gap=0.4):
    ...
    for i, (head, sub) in enumerate(items):
        x = ML + i * (bw + gap)
        last = i == n - 1
        rect(x, top, bw, h, fill=(GREEN_SOFT if last else white),
             line=(GREEN if last else HAIR), lw=(1.5 if last else 1))
        para(f"<b>{i+1}</b>", top + 0.18, x + 0.16, 0.5, 12, GREEN if last else MUTED, "ARB")
        para(f"<b>{head}</b>", top + 0.55, x + 0.16, bw - 0.3, 13, GREEN if last else INK, "ARB", lead=15)
        para(sub, top + h - 0.42, x + 0.16, bw - 0.3, 10, MUTED, lead=12)


def statcards(stats, top, h=1.35):
    n = len(stats); gap = 0.25
    swd = (CW - (n - 1) * gap) / n
...
def risk_line(points, x, top, w, h):
    ...
    # threshold
    ty = py + ph * (1 - THR)
    c.setStrokeColor(RED); c.setLineWidth(0.8); c.setDash(3, 3)
    c.line(X(px), Y(ty), X(px + pw), Y(ty)); c.setDash()
    ...
    if pts:
        ...
        c.setStrokeColor(GREEN); c.setLineWidth(0.9); c.setDash()
        c.drawPath(path)
...
para("Bir CNC ve lazer tesisi için uçtan uca duruş tahmini, kök neden ve What-If sistemi",
     4.45, ML, CW, 16, MUTED)
rect(ML, 5.2, 2.2, 0.025, fill=GREEN)
para("Veri dönemi Ağustos 2025 - Mayıs 2026  ·  12 makine  ·  yaklaşık 7,4 milyon telemetri kaydı",
     5.45, ML, CW, 12.5, MUTED)
para("trexCloud · Kestirimci OEE ve Kök Neden Analizi", 7.08, ML, 8, 9, MUTED)
page()
...
para("Sistem beş aşamadan oluşur. Her aşama bir öncekinin çıktısını kullanır.", 2.25, ML, CW, 14.5, INK)
numbox([
...
para("Telemetrisi olan ve duruşu kaydedilen yedi makine, OEE değerine göre sıralanmıştır. "
     "Yeşil Fanuc, kehribar Mitsubishi makineleri gösterir.", 2.25, ML, CW, 13.5, INK, lead=17)
...
], 3.55, x=7.62, w=4.6, size=10.5, gap=0.12)
page()
...
statcards([("0,73", "ROC-AUC · Fanuc"), ("2,38×", "Lift · Fanuc"),
           (f"%{DEP['recall']*100:.0f}", "Recall · held-out"), ("1,2×", "Lift · Mitsubishi")], 2.25, h=1.2)
# comparison table
rows = [
...
cols = [("Eğitim verisi", 3.0), ("Test", 1.7), ("ROC", 1.0), ("Lift", 1.0), ("Recall", 1.1), ("Not", 3.83)]
x = ML
...
for i, r in enumerate(rows):
    ...
    for j, (value, wd) in enumerate(vals):
        ...
        para(value, y + 0.13, x + 0.1, wd - 0.18, 10, GREEN if i == 0 else INK,
             "ARB" if i == 0 else "AR")
        x += wd
para("Fanuc hücresi {1,2,3,5,9} tek gerçek tahmin edilebilir gruptur. Mitsubishi makineleri zamanın "
     "yüzde 53'ünde zaten durur, bu yüzden yüksek recall değeri bilgi taşımaz ve lift yalnızca 1,2 katıdır.",
     6.35, ML, CW, 12, MUTED, lead=15.5)
page()

# ═══════════════════════════ 07 risk timeline (chart) ═══════════════════════════
base("Tahmin", "Modelin ürettiği risk zaman çizgisi", 7)
para("Aşağıdaki eğri, Makine 1 için modelin held-out dönemde ürettiği duruş riskidir. Bu, hiçbir "
     "müdahale yapılmadığında beklenen riski gösterir.", 2.25, ML, CW, 13.5, INK, lead=17)
...
# ═══════════════════════════ 08 bridge ═══════════════════════════
base("OEE Kazanımı", "Tahminden OEE kazanımına geçiş", 8)
para("Tahmin doğruluğunu OEE kazanımına çeviren hesap zinciri şudur.", 2.25, ML, CW, 14, INK)
...
base("Örüntü", "Makineler arası örüntü, senkron duruşlar", 9)
para("Soru şu: iki veya daha fazla makine aynı saatte plansız durduğunda, bu durum şansın ve ortak "
     "vardiya programının ötesinde bir bağ mıdır?", 2.25, ML, CW, 14, INK, lead=17)
bars = [("Gözlemlenen", SYNC["observed_co_stop_hours"], GREEN, "gerçekte yaşanan eş zamanlı duruş"),
...
for i, (lab, v, col, note) in enumerate(bars):
    ...
    para(f'<b>{v} saat</b>', y + 0.05, ML + 3.2 + (v / 760) * 5.4, 1.2, 11.5, INK, "ARB")
    para(note, y + 0.06, ML + 3.2 + (v / 760) * 5.4 + 1.0, 3.6, 10, MUTED)
para(f"İstatistik sonucu: z eşittir {tr(SYNC['daily']['z'])}, p küçüktür 0,001. Senkronizasyon ortak "
     "vardiya programıyla açıklanamıyor.", 5.0, ML, CW, 13.5, INK, lead=17)
rect(ML, 5.5, CW, 0.95, fill=GREEN_SOFT, line=GREEN, lw=1.2)
para("<b>İçgörü:</b> 708 saatlik eş zamanlı duruşun 564 saati ortak vardiya ile açıklanır. Geri kalan "
     ...
     5.68, ML + 0.22, CW - 0.44, 11.5, INK, lead=15)
page()

# ═══════════════════════════ 10 senaryolar ═══════════════════════════
base("Senaryolar", "What-If senaryoları, fiziksel kazanım", 10)
para("Her senaryo ham OEE bileşenlerinden aynı formülle yeniden hesaplanır. Değerler fiziksel "
     "kazanımdır; para birimi içermez.", 2.2, ML, CW, 13.5, INK, lead=17)
...
for label, wd in cols:
    ...
    x += wd
srows = [
    ("S1 · Makine 1 · en büyük plansız kalem -%30", "Kullanılabilirlik", "+18,0 pp", "625 saat çalışma", GREEN, GREEN_SOFT),
    ("S2 · Makine 5 · en büyük plansız kalem -%30", "Kullanılabilirlik", "+15,3 pp", "420 saat çalışma", INK, white),
    ("S3 · Makine 3 · çevrim süresi +%10", "Performans", "0,0 pp", "veri P kolunu desteklemiyor", MUTED, white),
    ("S4 · Tesis · bağlantı kesintisi giderilir", "Bağlantı / IT", "makine dışı", "663 saat bağlantı", MUTED, white),
]
for i, (name, lev, doee, recov, colr, fill) in enumerate(srows):
    ...
    for value, wd, align in vals:
        ...
        para(value, y + 0.13, x + 0.1, wd - 0.2, 10, colr, "ARB" if i == 0 else "AR", align=align)
        x += wd
bullets([
    ...
    "için inerttir.",
], 5.45, size=10.5, gap=0.11)
page()

# ═══════════════════════════ 11 finansal çerçeve ═══════════════════════════
base("Finansal Çerçeve", "Para birimi neden ayrı tutuluyor", 11)
bullets([
...
# ═══════════════════════════ 12 sınırlar ═══════════════════════════
base("Sınırlar", "Neyi yapamayacağımızı da söylüyoruz", 12)
bullets([
...
# ═══════════════════════════ 13 sonuç ═══════════════════════════
base("Sonuç", "Tahmin eder, açıklar, sayısallaştırır", 13)
statcards([("2,38×", "Fanuc duruş tahmini lift"),
           ("+10,4 puan", "kestirimci bakım OEE etkisi"),
           ("z = 5,16", "senkron duruş anlamlılığı"),
           ("+18 pp", "en iyi What-If senaryosu")], 2.45)
bullets([
